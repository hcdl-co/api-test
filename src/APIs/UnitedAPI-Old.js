...
const United = () => {
  ...
  function showAddress(e) {
    ...
    const fetchLocation = async () => {
      try {
        ...
        const json = await response.json();
        //we don't want to get the location data if it didn't get anything so we wrUniteded it in an if statement
        if (locationURLs === []) {
  ...
  function orgDisplay(json) {
    ...
    json.entry.forEach((provider) =>
      locURLArray.push(provider.resource.location[0].reference)
    );
  ...
  return (
    <div className="United">
      ...
      {buttonClicked && totalFound === 0 && noResultsCall ? (
        <div>
          <p className="notfound">
            We didn't find anything that matched that search criteria
          </p>
        </div>
      ) : null}
      ...
      {totalFound > 0 ? (
        <p>
          We found {totalFound} results matching that criteria. Showing{" "}
      ...
      {providerArray !== undefined && providerArray.length ? (
        <div>
            ...
            {providerArray.map((provider, index) => {
              return (
                <tbody key={provider.fullUrl}>
                  <tr>
                    <td key={provider.resource.name}>
                      {provider.resource.name}
                    </td>
                    <td key={providerSpecialtyList[index]}>
                      {providerSpecialtyList[index]}
                    </td>
                    <td key={provider.resource.telecom[0].value}>
                      {provider.resource.telecom[0].value}
                    </td>
                    <td>
                      <button key={index} value={index} onClick={showAddress}>
                        Show Address
                      </button>
                    </td>
                  </tr>
          ...
          {searchHolder.length > 1 && totalFound !== resultTracker ? (
            <button className="showButton" onClick={next}>
              Show More Results
            </button>
          ) : null}
        </div>
      ...
      {providerArray === undefined ? (
        <div>
          <p>
            Sorry, your search did not work, please try again or notify our team
            at info@healthcareDL.com
          </p>
        </div>
      ) : null}
    </div>
